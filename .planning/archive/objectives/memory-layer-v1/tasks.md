...
- [x] `MemoryStore` exists with the minimum methods defined in the design.
- [x] `MemoryItem`, `MemorySearchResult`, and `MemoryContextBundle` exist.
- [x] Contract tests define the expected behavior of the store surface.
...
- [x] `EngramMemoryStore` implements the contract methods needed for the first slice.
- [x] Engram-specific payload shapes are mapped into internal memory models.
- [x] No new caller outside the adapter depends on raw Engram tool semantics.
...
- [x] The minimum memory tables exist.
- [x] `PostgresMemoryStore` supports save/get/list/search basic flows.
- [x] Integration tests prove items, session summaries, and preferences can be persisted and retrieved.
...
- [x] At least one session summary flow uses `MemoryService`.
- [x] At least one learning/fix flow uses `MemoryService`.
- [x] At least one preference flow uses `MemoryService`.
- [x] The flows no longer depend conceptually on Engram.
...
- [x] Handoff reflects exact completion state and next recommended task.
- [x] Remaining work is explicitly queued for hybrid retrieval and eval phases.
