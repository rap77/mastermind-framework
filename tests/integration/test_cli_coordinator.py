...
def test_cli_coordinator_type_boundary():
    ...
    result = runner.invoke(orchestrate, [
        'run',
        '--brief', 'Test brief with sufficient length',
...
def test_cli_invalid_params():
    ...
    result = runner.invoke(orchestrate, [
        'run',
        '--brief', '',  # This will fail at Click level (empty string)
    ...
    has_iterations_error = 'max_iterations' in output_lower or 'max-iterations' in output_lower

    assert has_brief_error or has_iterations_error, f"Expected validation error, got: {result.output}"
