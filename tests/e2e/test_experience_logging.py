...
# Configure pytest-asyncio for this module
...
async def test_redact_for_storage_function():
    ...
    # Test with API key
    data_with_key = {"api_key": "sk-1234567890abcdef", "message": "Hello"}
    redacted = redact_for_storage(data_with_key)
    assert "[REDACTED_SECRET]" in redacted
    assert "sk-1234567890abcdef" not in redacted
    # Test with email
    data_with_email = {"email": "user@example.com", "message": "Hello"}
...
async def test_parent_outputs_lineage():
    ...
    recent_records = await logger.get_recent_by_brain(
        "brain-software-07-growth-data", limit=10
    )
...
async def test_pii_redaction_nested_structures():
    ...
    nested_data = {
        "user": {
            ...
            "address": {"zip": "12345", "ssn": "987-65-4321"},
        },
