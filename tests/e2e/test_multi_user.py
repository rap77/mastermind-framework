...
async def test_multi_user_isolation(app_with_mock_auth):
    ...
    with patch('mastermind_cli.api.routes.tasks.get_current_user_any', side_effect=mock_get_user_a):
        async with AsyncClient(transport=ASGITransport(app=app_with_mock_auth), base_url="http://test") as client_a:
    ...
    with patch('mastermind_cli.api.routes.tasks.get_current_user_any', side_effect=mock_get_user_b):
        async with AsyncClient(transport=ASGITransport(app=app_with_mock_auth), base_url="http://test") as client_b:
...
async def test_concurrent_task_creation(app_with_mock_auth):
    ...
    async def create_task(user_num):
        with patch('mastermind_cli.api.routes.tasks.get_current_user_any', side_effect=await mock_get_user_factory(user_num)()):
            async with AsyncClient(transport=ASGITransport(app=app_with_mock_auth), base_url="http://test") as client:
...
async def test_per_request_orchestrator_instances(app_with_mock_auth):
    ...
    with patch('mastermind_cli.api.routes.tasks.get_current_user_any', side_effect=mock_get_user):
        async with AsyncClient(transport=ASGITransport(app=app_with_mock_auth), base_url="http://test") as client:
            # Create tasks with different configurations
            task_1 = await client.post("/api/tasks", json={
            ...
            task_2 = await client.post("/api/tasks", json={
...
async def test_websocket_route_registered(app_with_mock_auth):
    ...
    assert any("/ws" in route for route in routes), "WebSocket route should be registered"
