...
class TestDiscoveryFlow:
    ...
    def test_detect_flow_with_short_brief(self):
        ...
        assert (
            flow == Coordinator.FLOW_DISCOVERY
        ), f"Expected discovery flow, got {flow}"
...
si hay market fit antes de invertir en desarrollo."""
    ...
    def test_generate_interview_plan(self):
        ...
        assert (
            plan.get("status") == "success"
        ), f"Plan status should be success, got {plan.get('status')}"
        ...
        assert (
            "initial_questions" in interview_strategy
        ), "Should have initial questions"
    ...
    def test_conduct_interview_basic(self):
        ...
        try:
            ...
            with patch(
                "builtins.input", return_value="My users are small business owners"
            ):
            ...
            assert (
                interview_doc["outcome"]["total_questions_asked"] >= 1
            ), "Should have asked at least 1 question"
    ...
    def test_output_formatter_discovery_methods(self):
        ...
        assert hasattr(
            formatter, "format_interview_plan"
        ), "Missing format_interview_plan"
        assert hasattr(
            formatter, "format_question_to_user"
        ), "Missing format_question_to_user"
        assert hasattr(
            formatter, "format_followup_response"
        ), "Missing format_followup_response"
        assert hasattr(
            formatter, "format_interview_complete"
        ), "Missing format_interview_complete"
    ...
    def test_discovery_flow_constant(self):
        ...
        assert (
            Coordinator.FLOW_DISCOVERY == "discovery"
        ), f"FLOW_DISCOVERY should be 'discovery', got {Coordinator.FLOW_DISCOVERY}"
