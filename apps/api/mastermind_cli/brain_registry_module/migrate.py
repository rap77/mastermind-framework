"""
Brain Registry migration runner.

Applies SQL migrations from the migrations/ directory to PostgreSQL.
Tracks applied migrations in a schema_migrations table to avoid re-runs.

Usage:
    uv run python -m mastermind_cli.brain_registry_module.migrate
    DATABASE_URL=postgresql://... uv run python -m mastermind_cli.brain_registry_module.migrate
"""
...
async def run_migrations(database_url: str = _DEFAULT_DATABASE_URL) -> list[str]:
    ...
    Reads *.sql files from the migrations/ directory, skips already-applied
    ones, and records each in schema_migrations after successful execution.
...
def main() -> None:
    """Entry point for running migrations from the CLI."""
