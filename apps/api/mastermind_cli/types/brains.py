...
class StandardSchema(BaseModel):
    ...
    raw_fallback: Optional[str] = Field(
        None, description="Fallback for unparseable output"
    )
...
def normalize_brain_output(raw_yaml: str) -> StandardSchema:
    ...
    try:
        ...
        return StandardSchema(brain_id="parse_error", content="", raw_fallback=raw_yaml)
