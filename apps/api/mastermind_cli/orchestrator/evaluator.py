...
class Evaluator:
    ...
    def __init__(self, skills_dir: Optional[str] = None) -> None:
        ...
        if skills_dir is None:
            ...
            self.skills_dir = project_root / "skills" / "evaluator"
        else:
    ...
    def load_matrix(self, matrix_id: str) -> dict[str, Any]:
    ...
    def _list_matrices(self) -> List[str]:
        ...
        return [f"MATRIX-{f.stem}" for f in self.matrices_dir.glob("*.yaml")]
    ...
    def evaluate(
        ...
        return {
            ...
            "summary": self._generate_summary(
                veredict, score_percentage, passed, failed
            ),
            "passed_checks": passed,
            "failed_checks": failed,
            "biases_detected": biases,
            "redirect_instructions": self._generate_redirect(
                veredict, failed, brain_id
            ),
        }

    def _evaluate_check(
        self, check: dict[str, Any], output: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        evidence_found = self._look_for_evidence(
            check_id, check_description, output_text
        )
    ...
    def _look_for_evidence(
        self, check_id: str, check_desc: str, output_text: str
    ) -> Optional[str]:
    ...
    def _detect_biases(
        self, output: dict[str, Any], failed_checks: List[dict[str, Any]]
    ) -> List[dict[str, Any]]:
...
{failed_count} critical issues found. The output needs fundamental rework."""
    ...
    def _generate_redirect(
        self, veredict: str, failed: List[dict[str, Any]], brain_id: int
        ...
        fixes = [
            check["fix_instruction"] for check in failed if check.get("fix_instruction")
        ]
