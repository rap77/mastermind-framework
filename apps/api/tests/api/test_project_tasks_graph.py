"""API tests for project task list and dependency endpoints."""

from __future__ import annotations

from datetime import datetime, timedelta, timezone
from pathlib import Path

import pytest
from fastapi import FastAPI
from httpx import ASGITransport, AsyncClient

from mastermind_cli.api.app import create_app
from mastermind_cli.api.dependencies import get_db_path, get_project_state_db_url
from mastermind_cli.api.routes.auth import create_access_token
from mastermind_cli.project_state.database.session import (
    dispose_engines,
    get_session_factory,
    initialize_database,
)
from mastermind_cli.project_state.models.project import Project
from mastermind_cli.project_state.models.task import Task
from mastermind_cli.project_state.models.task_dependency import TaskDependency


def _build_test_app(db_path: str, project_state_db_url: str) -> FastAPI:
    """Create an app instance wired to temporary test databases."""
    app = create_app(db_path)

    async def _override_db_path() -> str:
        """Provide the test task-state database path."""
        return db_path

    async def _override_project_state_db_url() -> str:
        """Provide the test project-state database URL."""
        return project_state_db_url

    app.dependency_overrides[get_db_path] = _override_db_path
    app.dependency_overrides[get_project_state_db_url] = _override_project_state_db_url
    return app


@pytest.mark.asyncio
async def test_project_task_list_returns_recent_tasks(tmp_path: Path) -> None:
    """Return project tasks ordered by most recent update."""
    db_path = str(tmp_path / "test.db")
    database_url = f"sqlite:///{db_path}.project_state"
    dispose_engines()
    initialize_database(database_url)
    app = _build_test_app(db_path, database_url)
    auth_headers = {
        "Authorization": f"Bearer {create_access_token('test-user-id-001')}"
    }
    session_factory = get_session_factory(database_url)
    now = datetime.now(timezone.utc)

    with session_factory() as session:
        session.add(
            Project(
                project_id="project-tasks",
                name="Project Tasks",
                status="active",
                adapter_id="adapter-tasks",
                metadata_json={},
                created_at=now,
                updated_at=now,
            )
        )
        session.add_all(
            [
                Task(
                    task_id="task-new",
                    project_id="project-tasks",
                    title="Newest task",
                    status="in_progress",
                    priority="high",
                    owner_type="agent",
                    owner_id="brain-04",
                    metadata_json={},
                    constraints={},
                    completion_criteria={},
                    created_at=now,
                    updated_at=now,
                ),
                Task(
                    task_id="task-old",
                    project_id="project-tasks",
                    title="Older task",
                    status="pending",
                    priority="normal",
                    owner_type=None,
                    owner_id=None,
                    metadata_json={},
                    constraints={},
                    completion_criteria={},
                    created_at=now - timedelta(minutes=5),
                    updated_at=now - timedelta(minutes=5),
                ),
            ]
        )
        session.commit()

    async with AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        response = await client.get(
            "/api/projects/project-tasks/tasks?limit=10",
            headers=auth_headers,
        )

    assert response.status_code == 200
    body = response.json()
    assert [item["task_id"] for item in body["tasks"]] == ["task-new", "task-old"]


@pytest.mark.asyncio
async def test_project_task_dependencies_returns_seeded_edges(tmp_path: Path) -> None:
    """Return task dependency edges for a seeded task."""
    db_path = str(tmp_path / "test.db")
    database_url = f"sqlite:///{db_path}.project_state"
    dispose_engines()
    initialize_database(database_url)
    app = _build_test_app(db_path, database_url)
    auth_headers = {
        "Authorization": f"Bearer {create_access_token('test-user-id-001')}"
    }
    session_factory = get_session_factory(database_url)
    now = datetime.now(timezone.utc)

    with session_factory() as session:
        session.add(
            Project(
                project_id="project-deps",
                name="Project Deps",
                status="active",
                adapter_id="adapter-deps",
                metadata_json={},
                created_at=now,
                updated_at=now,
            )
        )
        session.add_all(
            [
                Task(
                    task_id="task-main",
                    project_id="project-deps",
                    title="Main task",
                    status="in_progress",
                    priority="high",
                    owner_type="agent",
                    owner_id="brain-04",
                    metadata_json={},
                    constraints={},
                    completion_criteria={},
                    created_at=now,
                    updated_at=now,
                ),
                Task(
                    task_id="task-prereq",
                    project_id="project-deps",
                    title="Prerequisite task",
                    status="pending",
                    priority="normal",
                    owner_type=None,
                    owner_id=None,
                    metadata_json={},
                    constraints={},
                    completion_criteria={},
                    created_at=now,
                    updated_at=now,
                ),
            ]
        )
        session.add(
            TaskDependency(
                dependency_id="dep-1",
                task_id="task-main",
                depends_on_task_id="task-prereq",
                dependency_type="blocks",
                created_at=now,
            )
        )
        session.commit()

    async with AsyncClient(
        transport=ASGITransport(app=app), base_url="http://test"
    ) as client:
        response = await client.get(
            "/api/projects/project-deps/tasks/task-main/dependencies",
            headers=auth_headers,
        )

    assert response.status_code == 200
    body = response.json()
    assert body["task_id"] == "task-main"
    assert len(body["dependencies"]) == 1
    assert body["dependencies"][0]["depends_on_task_id"] == "task-prereq"
