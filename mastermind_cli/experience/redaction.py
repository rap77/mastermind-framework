...
PII_PATTERNS = [
    (
        ...
        "[REDACTED_SECRET]",
    ),  # OpenAI/Stripe keys (reduced min length for tests)
    (
        re.compile(r"mmsk_[a-zA-Z0-9]{10,}", re.IGNORECASE),
        "[REDACTED_SECRET]",
    ),  # MultiOn keys (reduced min length for tests)
    (
        re.compile(
...
def redact_dict(
    data: Dict[str, Any], seen: Optional[set[int]] = None
) -> Dict[str, Any]:
    ...
    for key, value in data.items():
        if isinstance(value, str):
            ...
            redacted[key] = [
                redact_dict(v, seen) if isinstance(v, dict) else v for v in value
            ]
