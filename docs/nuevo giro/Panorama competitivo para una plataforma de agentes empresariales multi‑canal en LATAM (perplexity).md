...
```mermaid
flowchart TD
    U[Usuario con idea de producto] -->|Describe idea en la web app| D[Agente de Descubrimiento]
    D -->|Genera visión y requisitos| V[Documento de Visión]
    V --> A[Agente Arquitecto de Plataforma]
    A -->|Diseña módulos y stack| ARQ[Propuesta de Arquitectura MVP]
    ARQ --> P[Agente Planificador / Roadmap]
    P -->|Fases + backlog inicial| PLAN[Roadmap y Backlog]
    PLAN -->|Opcional: definir qué agentes internos se necesitan| F[Agente Creador de Agentes]
    F -->|Crea fichas de nuevos agentes| AGENTS[Catálogo de Agentes]
    AGENTS --> O[Agente Orquestador]
    O -->|Ejecuta y coordina agentes en el tiempo| R[Proyecto en ejecución]
    R -->|Logs y resultados de agentes| E[Agente Evaluador de Agentes]
    E -->|Sugerencias de mejora de agentes| O
```
---
...
```mermaid
flowchart TD
    S[Inicio: Web app] --> NP[Nuevo proyecto]
    NP --> CHAT[Chat descubrimiento]
    CHAT --> RESUMEN[Resumen vision]
    RESUMEN --> ARQ_VIEW[Vista arquitectura]
    ARQ_VIEW --> PLAN_VIEW[Vista roadmap y backlog]
    PLAN_VIEW --> AGENT_VIEW[Vista agentes]
    AGENT_VIEW --> CICLO[Proyecto listo para ejecucion]
```
